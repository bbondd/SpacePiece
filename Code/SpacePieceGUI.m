function varargout = SpacePieceGUI(varargin)
% SPACEPIECEGUI MATLAB code for SpacePieceGUI.fig
%      SPACEPIECEGUI, by itself, creates a new SPACEPIECEGUI or raises the existing
%      singleton*.
%
%      H = SPACEPIECEGUI returns the handle to a new SPACEPIECEGUI or the handle to
%      the existing singleton*.
%
%      SPACEPIECEGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SPACEPIECEGUI.M with the given input arguments.
%
%      SPACEPIECEGUI('Property','Value',...) creates a new SPACEPIECEGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SpacePieceGUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SpacePieceGUI_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @SpacePieceGUI_OpeningFcn, ...
                   'gui_OutputFcn',  @SpacePieceGUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SpacePieceGUI is made visible.
function SpacePieceGUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SpacePieceGUI (see VARARGIN)

% Choose default command line output for SpacePieceGUI
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes SpacePieceGUI wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SpacePieceGUI_OutputFcn(hObject, eventdata, handles) 
...
angle1 = pi * str2double(get(handles.angle1, 'String'));
angle2 = pi * str2double(get(handles.angle2, 'String'));
angle3 = pi * str2double(get(handles.angle3, 'String'));
imagePath1 = get(handles.imagePath1, 'String');
imagePath2 = get(handles.imagePath2, 'String');
imagePath3 = get(handles.imagePath3, 'String');

vectors = [x1 x2 x3; y1 y2 y3; z1 z2 z3];
angles = [angle1 angle2 angle3];
imagePaths = char(imagePath1, imagePath2, imagePath3); 
main(vectors, angles, imagePaths);
...
imagePath1 = sprintf('%s%s', path, file);
set(handles.imagePath1, 'String', imagePath1);
...
[file, path] = uigetfile('*.png');
imagePath2 = sprintf('%s%s', path, file);
set(handles.imagePath2, 'String', imagePath2);
% hObject    handle to path2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


% --- Executes on button press in path3.
function path3_Callback(hObject, eventdata, handles)
[file, path] = uigetfile('*.png');
imagePath3 = sprintf('%s%s', path, file);
set(handles.imagePath3, 'String', imagePath3);
